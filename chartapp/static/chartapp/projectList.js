...
const namingForm = document.getElementById('project-form');
...
function createProject() {
  let data = {'name': clearForm(namingForm)}
  fetchData(data)
}

function deleteProject(value) {
  let data = {'delete': value}
  fetchData(data)
}

function updateProject(value) {
  let formValue = clearForm(namingForm)
  let data = {'update': value, 'rename': formValue}
  fetchData(data)
}
...
mainModal.addEventListener('show.bs.modal', function (event) {
  ...
  if (targetId.id === 'create-btn') {
    titleModal.innerHTML = 'New Project';
    ...
    titleModal.innerHTML = 'Rename Project';
    let btn = document.getElementById('change');
    let btnCln = btn.cloneNode(true);
    btnCln.innerHTML = 'Save';
    btnCln.addEventListener('click', function(){
      updateProject(event.relatedTarget.value);
    });
    btn.parentNode.appendChild(btnCln)
    btn.remove()

  } else if (targetId.id === 'delete-btn') {
    titleModal.innerHTML = 'Delete Project';
    let btn = document.getElementById('change');
    let btnCln = btn.cloneNode(true);
    btnCln.innerHTML = 'Delete';
    btnCln.className = 'btn btn-danger';
    btnCln.addEventListener('click', function(){
      deleteProject(event.relatedTarget.value);
    });
    btn.parentNode.appendChild(btnCln)
    btn.remove()
    document.getElementById('form').style.display = 'none';
    document.getElementById('content').innerHTML = 'Are you sure you want to delete this project?'
  };
});
